...
static char *ov5648_hardware_info="unknown ov5648 unknown_5M_FF";
#endif
...
struct st_ov5648_otp st_ov5648_default_otp={
...
#define BG_Ratio_Typical_ov5648 0x0129
#define RG_Ratio_Typical_ov5648 0x013E
// modified by yangze for ov5648 AWB typical value (x825) 2013-08-28 end

#define OTP_DEBUG_EN 0

static int16_t ov5648_check_otp(struct msm_sensor_ctrl_t *s_ctrl, int16_t index){
	int16_t flag, i;
	int16_t rg, bg;
...
static int16_t ov5648_read_otp(struct msm_sensor_ctrl_t *s_ctrl, int16_t index, struct st_ov5648_otp *otp_ptr){
	int16_t i;
	int16_t temp1, temp2;
...
static int16_t ov5648_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl, int16_t R_gain, int16_t G_gain, int16_t B_gain){
...
static void ov5648_start_setting(struct msm_sensor_ctrl_t * s_ctrl){
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,0x0100,0x01,MSM_CAMERA_I2C_BYTE_DATA);
}

static void ov5648_stop_setting(struct msm_sensor_ctrl_t * s_ctrl){
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,0x0100,0x00,MSM_CAMERA_I2C_BYTE_DATA);
}	

static int16_t ov5648_update_otp_awb(struct msm_sensor_ctrl_t * s_ctrl){
	struct st_ov5648_otp current_otp;
	int16_t i;
	int16_t otp_index;
	int16_t temp;
	int16_t R_gain;
	...
	for(i=1;i<=3;i++){
		temp = ov5648_check_otp(s_ctrl, i);
		if(temp == 2){
			otp_index = i;
			break;
		}
	...
	if(i>3){
		current_otp.module_integrator_id = st_ov5648_default_otp.module_integrator_id;
		current_otp.lens_id = st_ov5648_default_otp.lens_id;
		current_otp.rg_ratio = st_ov5648_default_otp.rg_ratio;
		current_otp.bg_ratio = st_ov5648_default_otp.bg_ratio;
		current_otp.user_data[0] = st_ov5648_default_otp.user_data[0];
		current_otp.user_data[1] = st_ov5648_default_otp.user_data[1];
		current_otp.light_rg = st_ov5648_default_otp.light_rg;
		current_otp.light_bg= st_ov5648_default_otp.light_bg;
	}else{
		ov5648_read_otp(s_ctrl, otp_index, &current_otp);
	}
	...
	if(bg<BG_Ratio_Typical_ov5648){
		if(rg<RG_Ratio_Typical_ov5648){
			G_gain = 0x400;
			B_gain = 0x400*BG_Ratio_Typical_ov5648/bg;
			R_gain = 0x400*RG_Ratio_Typical_ov5648/rg;
		}else{
			R_gain = 0x400;
			G_gain = 0x400*rg/RG_Ratio_Typical_ov5648;
			B_gain = G_gain*BG_Ratio_Typical_ov5648/bg;
		}	
	}else{
		if(rg<RG_Ratio_Typical_ov5648){
			B_gain = 0x400;
			G_gain = 0x400*bg/BG_Ratio_Typical_ov5648;
			R_gain = G_gain*RG_Ratio_Typical_ov5648/rg;
		}else{
			G_gain_B = 0x400*bg/BG_Ratio_Typical_ov5648;
			G_gain_R = 0x400*rg/RG_Ratio_Typical_ov5648;
	...
	ov5648_update_awb_gain(s_ctrl, R_gain, G_gain, B_gain);
...
static void ov5648_otp_init_setting(struct msm_sensor_ctrl_t *s_ctrl){
...
static void ov5648_update_otp(struct msm_sensor_ctrl_t *s_ctrl){
	...
	ov5648_start_setting(s_ctrl);

	rc = ov5648_update_otp_awb(s_ctrl);
	if(rc == 0){
...
static struct msm_sensor_fn_t ov5648_sensor_func_tbl = {
	...
	.sensor_otp_init_setting = ov5648_otp_init_setting,
...
static int32_t ov5648_platform_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_GET_HARDWARE_INFO
	if(rc == 0){
		register_hardware_info(SUB_CAM, ov5648_hardware_info);
	}
	#endif
	// added by yangze for camera sensor hardware_info 2013-08-30 end
	return rc;
