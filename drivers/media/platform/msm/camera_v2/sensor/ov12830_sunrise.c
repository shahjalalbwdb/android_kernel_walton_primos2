...
#define OV12830_SUNRISE_SENSOR_NAME "ov12830_sunrise"
DEFINE_MSM_MUTEX(ov12830_sunrise_mut);
...
static char *ov12830_sunrise_hardware_info="sunrise ov12830 BW901_13M_AF";
#endif
/* Added by yangze for camera sensor hardware_info 2013-08-30 end */

static struct msm_sensor_ctrl_t ov12830_sunrise_s_ctrl;

static struct msm_sensor_power_setting ov12830_sunrise_power_setting[] = {
	{
...
static struct v4l2_subdev_info ov12830_sunrise_subdev_info[] = {
	{
...
static const struct i2c_device_id ov12830_sunrise_i2c_id[] = {
	{OV12830_SUNRISE_SENSOR_NAME,
	(kernel_ulong_t)&ov12830_sunrise_s_ctrl},
	{ }
};

static int32_t msm_ov12830_sunrise_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	return msm_sensor_i2c_probe(client, id, &ov12830_sunrise_s_ctrl);
}

static struct i2c_driver ov12830_sunrise_i2c_driver = {
	.id_table = ov12830_sunrise_i2c_id,
	.probe  = msm_ov12830_sunrise_i2c_probe,
	.driver = {
		.name = OV12830_SUNRISE_SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client ov12830_sunrise_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_WORD_ADDR,
};

static const struct of_device_id ov12830_sunrise_dt_match[] = {
	{.compatible = "qcom,ov12830_sunrise",
	.data = &ov12830_sunrise_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, ov12830_sunrise_dt_match);

static struct platform_driver ov12830_sunrise_platform_driver = {
	.driver = {
		.name = "qcom,ov12830_sunrise",
		.owner = THIS_MODULE,
		.of_match_table = ov12830_sunrise_dt_match,
	},
};

static int32_t ov12830_sunrise_platform_probe
	(struct platform_device *pdev)
{
	...
	match = of_match_device(ov12830_sunrise_dt_match, &pdev->dev);
	rc = msm_sensor_platform_probe(pdev, match->data);

	// added by yangze for camera sensor hardware_info 2013-08-30 begin
	#ifdef CONFIG_GET_HARDWARE_INFO
	if(rc == 0){
		register_hardware_info(MAIN_CAM, ov12830_sunrise_hardware_info);
	}
	#endif
	// added by yangze for camera sensor hardware_info 2013-08-30 end
	return rc;
...
static int __init ov12830_sunrise_init_module(void)
{
	int32_t rc = 0;
	pr_info("%s:%d\n", __func__, __LINE__);
	rc = platform_driver_probe(&ov12830_sunrise_platform_driver,
		ov12830_sunrise_platform_probe);
	if (!rc)
		return rc;
	pr_err("%s:%d rc %d\n", __func__, __LINE__, rc);
	return i2c_add_driver(&ov12830_sunrise_i2c_driver);
}

static void __exit ov12830_sunrise_exit_module(void)
{
	pr_info("%s:%d\n", __func__, __LINE__);
	if (ov12830_sunrise_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&ov12830_sunrise_s_ctrl);
		platform_driver_unregister
			(&ov12830_sunrise_platform_driver);
	} else
		i2c_del_driver(&ov12830_sunrise_i2c_driver);
	return;
}

// add by yangze for camera sensor otp func (x825) 2013-08-20 begin
struct st_ov12830_sunrise_otp{
...
static int16_t ov12830_sunrise_default_rg_ratio = 0x146;
static int16_t ov12830_sunrise_default_bg_ratio = 0x146;
static int16_t ov12830_sunrise_default_light_rg = 0x201;
static int16_t ov12830_sunrise_default_light_bg= 0x201;

static int16_t ov12830_sunrise_default_lenc[62] = {
...
#define BG_Ratio_Typical_ov12830_sunrise 0x0151
#define RG_Ratio_Typical_ov12830_sunrise 0x0136
// modified by yangze for ov12830 AWB typical value (x825) 2013-08-28 end

#define OTP_DEBUG_EN 0

static int16_t ov12830_sunrise_check_otp_wb(struct msm_sensor_ctrl_t *s_ctrl, int16_t index){
	int16_t flag, i;
	int16_t bank;
	int16_t address;
...
static int16_t ov12830_sunrise_check_otp_lenc(struct msm_sensor_ctrl_t *s_ctrl, int16_t index){
	int16_t flag, i;
	int16_t bank;
	int16_t address;
...
static int16_t ov12830_sunrise_read_otp_wb(struct msm_sensor_ctrl_t *s_ctrl, int16_t index, struct st_ov12830_sunrise_otp *otp_ptr){
	int16_t i;
	int16_t bank;
	int16_t address;
	int16_t temp1, temp2;
...
static int16_t ov12830_sunrise_read_otp_lenc(struct msm_sensor_ctrl_t *s_ctrl, int16_t index, struct st_ov12830_sunrise_otp *otp_ptr){
	int16_t i,j,k,l=0;
	int16_t bank;
	int16_t address;
...
static int16_t ov12830_sunrise_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl, int16_t R_gain, int16_t G_gain, int16_t B_gain){
...
static int16_t ov12830_sunrise_update_lenc(struct msm_sensor_ctrl_t *s_ctrl, struct st_ov12830_sunrise_otp *otp_ptr){
	int16_t i,temp;
...
static int16_t ov12830_sunrise_update_otp_wb(struct msm_sensor_ctrl_t * s_ctrl){
	struct st_ov12830_sunrise_otp current_otp;
	int16_t i;
	int16_t otp_index;
	int16_t temp;
	int16_t R_gain;
	...
	for(i=1;i<=3;i++){
		temp = ov12830_sunrise_check_otp_wb(s_ctrl, i);
		if(temp == 2){
			otp_index = i;
			break;
		}
	...
	if(i>3){
		current_otp.rg_ratio = ov12830_sunrise_default_rg_ratio;
		current_otp.bg_ratio = ov12830_sunrise_default_bg_ratio;
		current_otp.light_rg = ov12830_sunrise_default_light_rg;
		current_otp.light_bg= ov12830_sunrise_default_light_bg;
	}else{
		ov12830_sunrise_read_otp_wb(s_ctrl, otp_index, &current_otp);
	}
	...
	if(bg<BG_Ratio_Typical_ov12830_sunrise){
		if(rg<RG_Ratio_Typical_ov12830_sunrise){
			G_gain = 0x400;
			B_gain = 0x400*BG_Ratio_Typical_ov12830_sunrise/bg;
			R_gain = 0x400*RG_Ratio_Typical_ov12830_sunrise/rg;
		}else{
			R_gain = 0x400;
			G_gain = 0x400*rg/RG_Ratio_Typical_ov12830_sunrise;
			B_gain = G_gain*BG_Ratio_Typical_ov12830_sunrise/bg;
		}	
	}else{
		if(rg<RG_Ratio_Typical_ov12830_sunrise){
			B_gain = 0x400;
			G_gain = 0x400*bg/BG_Ratio_Typical_ov12830_sunrise;
			R_gain = G_gain*RG_Ratio_Typical_ov12830_sunrise/rg;
		}else{
			G_gain_B = 0x400*bg/BG_Ratio_Typical_ov12830_sunrise;
			G_gain_R = 0x400*rg/RG_Ratio_Typical_ov12830_sunrise;
	...
	ov12830_sunrise_update_awb_gain(s_ctrl, R_gain, G_gain, B_gain);
...
static int16_t ov12830_sunrise_update_otp_lenc(struct msm_sensor_ctrl_t * s_ctrl){
	struct st_ov12830_sunrise_otp current_otp;
	int16_t i,j;
	int16_t otp_index;
	int16_t temp;
	...
	for(i=1;i<=3;i++){
		temp = ov12830_sunrise_check_otp_lenc(s_ctrl, i);
		if(temp == 2){
			otp_index = i;
			break;
		}
	...
	if(i>3){
		for(j=0;j<62;j++){
			current_otp.lenc[i] = ov12830_sunrise_default_lenc[i];
		}
	}else{
		ov12830_sunrise_read_otp_lenc(s_ctrl, otp_index, &current_otp);
	}

	ov12830_sunrise_update_lenc(s_ctrl, &current_otp);
...
static int16_t ov12830_sunrise_update_blc_ratio(struct msm_sensor_ctrl_t * s_ctrl){
	int16_t k;
	int16_t temp;
...
static void ov12830_sunrise_start_setting(struct msm_sensor_ctrl_t * s_ctrl){
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,0x0100,0x01,MSM_CAMERA_I2C_BYTE_DATA);
}

static void ov12830_sunrise_stop_setting(struct msm_sensor_ctrl_t * s_ctrl){
...
static void ov12830_sunrise_otp_init_setting(struct msm_sensor_ctrl_t *s_ctrl){
...
static void ov12830_sunrise_update_otp(struct msm_sensor_ctrl_t *s_ctrl){
	...
	ov12830_sunrise_start_setting(s_ctrl);

	rc = ov12830_sunrise_update_otp_wb(s_ctrl);
	if(rc == 0){
	...
	rc = ov12830_sunrise_update_otp_lenc(s_ctrl);
	if(rc == 0){
	...
	rc = ov12830_sunrise_update_blc_ratio(s_ctrl);
	if(rc == 2){
	...
	ov12830_sunrise_stop_setting(s_ctrl);
	// add by yangze for camera sensor otp func (x825) 2013-08-20 end

	return;
}

static struct msm_sensor_fn_t ov12830_sunrise_sensor_func_tbl = {
	.sensor_config = msm_sensor_config,
	.sensor_power_up = msm_sensor_power_up,
	.sensor_power_down = msm_sensor_power_down,
	.sensor_match_id = msm_sensor_match_id,
	.sensor_otp_init_setting = ov12830_sunrise_otp_init_setting,
	.sensor_update_otp = ov12830_sunrise_update_otp,
};
// add by yangze for camera sensor otp func test (x825) 2013-08-19 end
static struct msm_sensor_ctrl_t ov12830_sunrise_s_ctrl = {
	.sensor_i2c_client = &ov12830_sunrise_sensor_i2c_client,
	.power_setting_array.power_setting = ov12830_sunrise_power_setting,
	.power_setting_array.size =
		ARRAY_SIZE(ov12830_sunrise_power_setting),
	.msm_sensor_mutex = &ov12830_sunrise_mut,
	.sensor_v4l2_subdev_info = ov12830_sunrise_subdev_info,
	.sensor_v4l2_subdev_info_size =
		ARRAY_SIZE(ov12830_sunrise_subdev_info),
	// add by yangze for camera sensor otp func test (x825) 2013-08-19
	.func_tbl = &ov12830_sunrise_sensor_func_tbl,
};

module_init(ov12830_sunrise_init_module);
module_exit(ov12830_sunrise_exit_module);
MODULE_DESCRIPTION("ov12830_sunrise");
