...
static char *ov12830_hardware_info="unknown ov12830 unknown_13M_AF";
#endif
...
static int32_t msm_ov12830_i2c_probe(struct i2c_client *client,
	...
	(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_GET_HARDWARE_INFO
	if(rc == 0){
		register_hardware_info(MAIN_CAM, ov12830_hardware_info);
	}
	#endif
	// added by yangze for camera sensor hardware_info 2013-08-30 end
...
static int16_t ov12830_default_rg_ratio = 0x146;
static int16_t ov12830_default_bg_ratio = 0x146;
static int16_t ov12830_default_light_rg = 0x201;
static int16_t ov12830_default_light_bg= 0x201;

static int16_t ov12830_default_lenc[62] = {
...
#define BG_Ratio_Typical_ov12830 0x0151
#define RG_Ratio_Typical_ov12830 0x0136
// modified by yangze for ov12830 AWB typical value (x825) 2013-08-28 end

#define OTP_DEBUG_EN 0

static int16_t ov12830_check_otp_wb(struct msm_sensor_ctrl_t *s_ctrl, int16_t index){
	int16_t flag, i;
	int16_t bank;
	int16_t address;
...
static int16_t ov12830_check_otp_lenc(struct msm_sensor_ctrl_t *s_ctrl, int16_t index){
	int16_t flag, i;
	int16_t bank;
	int16_t address;
...
static int16_t ov12830_read_otp_wb(struct msm_sensor_ctrl_t *s_ctrl, int16_t index, struct st_ov12830_otp *otp_ptr){
	int16_t i;
	int16_t bank;
	int16_t address;
	int16_t temp1, temp2;
...
static int16_t ov12830_read_otp_lenc(struct msm_sensor_ctrl_t *s_ctrl, int16_t index, struct st_ov12830_otp *otp_ptr){
	int16_t i,j,k,l=0;
	int16_t bank;
	int16_t address;
...
static int16_t ov12830_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl, int16_t R_gain, int16_t G_gain, int16_t B_gain){
...
static int16_t ov12830_update_lenc(struct msm_sensor_ctrl_t *s_ctrl, struct st_ov12830_otp *otp_ptr){
	int16_t i,temp;
...
static int16_t ov12830_update_otp_wb(struct msm_sensor_ctrl_t * s_ctrl){
	struct st_ov12830_otp current_otp;
	int16_t i;
	int16_t otp_index;
	int16_t temp;
	int16_t R_gain;
	...
	for(i=1;i<=3;i++){
		temp = ov12830_check_otp_wb(s_ctrl, i);
		if(temp == 2){
			otp_index = i;
			break;
		}
	...
	if(i>3){
		current_otp.rg_ratio = ov12830_default_rg_ratio;
		current_otp.bg_ratio = ov12830_default_bg_ratio;
		current_otp.light_rg = ov12830_default_light_rg;
		current_otp.light_bg= ov12830_default_light_bg;
	}else{
		ov12830_read_otp_wb(s_ctrl, otp_index, &current_otp);
	}
	...
	if(bg<BG_Ratio_Typical_ov12830){
		if(rg<RG_Ratio_Typical_ov12830){
			G_gain = 0x400;
			B_gain = 0x400*BG_Ratio_Typical_ov12830/bg;
			R_gain = 0x400*RG_Ratio_Typical_ov12830/rg;
		}else{
			R_gain = 0x400;
			G_gain = 0x400*rg/RG_Ratio_Typical_ov12830;
			B_gain = G_gain*BG_Ratio_Typical_ov12830/bg;
		}	
	}else{
		if(rg<RG_Ratio_Typical_ov12830){
			B_gain = 0x400;
			G_gain = 0x400*bg/BG_Ratio_Typical_ov12830;
			R_gain = G_gain*RG_Ratio_Typical_ov12830/rg;
		}else{
			G_gain_B = 0x400*bg/BG_Ratio_Typical_ov12830;
			G_gain_R = 0x400*rg/RG_Ratio_Typical_ov12830;
	...
	ov12830_update_awb_gain(s_ctrl, R_gain, G_gain, B_gain);
...
static int16_t ov12830_update_otp_lenc(struct msm_sensor_ctrl_t * s_ctrl){
	struct st_ov12830_otp current_otp;
	int16_t i,j;
	int16_t otp_index;
	int16_t temp;
	...
	for(i=1;i<=3;i++){
		temp = ov12830_check_otp_lenc(s_ctrl, i);
		if(temp == 2){
			otp_index = i;
			break;
		}
	...
	if(i>3){
		for(j=0;j<62;j++){
			current_otp.lenc[i] = ov12830_default_lenc[i];
		}
	}else{
		ov12830_read_otp_lenc(s_ctrl, otp_index, &current_otp);
	}

	ov12830_update_lenc(s_ctrl, &current_otp);
...
static int16_t ov12830_update_blc_ratio(struct msm_sensor_ctrl_t * s_ctrl){
	int16_t k;
	int16_t temp;
...
static void ov12830_start_setting(struct msm_sensor_ctrl_t * s_ctrl){
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,0x0100,0x01,MSM_CAMERA_I2C_BYTE_DATA);
}

static void ov12830_stop_setting(struct msm_sensor_ctrl_t * s_ctrl){
...
static void ov12830_otp_init_setting(struct msm_sensor_ctrl_t *s_ctrl){
...
static void ov12830_update_otp(struct msm_sensor_ctrl_t *s_ctrl){
	...
	ov12830_start_setting(s_ctrl);

	rc = ov12830_update_otp_wb(s_ctrl);
	if(rc == 0){
	...
	rc = ov12830_update_otp_lenc(s_ctrl);
	if(rc == 0){
	...
	rc = ov12830_update_blc_ratio(s_ctrl);
	if(rc == 2){
	...
	ov12830_stop_setting(s_ctrl);
	// add by yangze for camera sensor otp func (x825) 2013-08-20 end
	
	return;
}

static struct msm_sensor_fn_t ov12830_sensor_func_tbl = {
	.sensor_config = msm_sensor_config,
	.sensor_power_up = msm_sensor_power_up,
	.sensor_power_down = msm_sensor_power_down,
	.sensor_match_id = msm_sensor_match_id,
	.sensor_otp_init_setting = ov12830_otp_init_setting,
	.sensor_update_otp = ov12830_update_otp,
};
...
static struct msm_sensor_ctrl_t ov12830_s_ctrl = {
	...
	.func_tbl = &ov12830_sensor_func_tbl,
};
