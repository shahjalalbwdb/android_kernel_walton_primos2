...
#define OV5648_SUNRISE_SENSOR_NAME "ov5648_sunrise"
DEFINE_MSM_MUTEX(ov5648_sunrise_mut);
...
static char *ov5648_sunrise_hardware_info="sunrise ov5648 BW901_5M_FF";
#endif
/* Added by yangze for camera sensor hardware_info 2013-08-30 end */

static struct msm_sensor_ctrl_t ov5648_sunrise_s_ctrl;

static struct msm_sensor_power_setting ov5648_sunrise_power_setting[] = {
	{
...
static struct v4l2_subdev_info ov5648_sunrise_subdev_info[] = {
	{
...
static const struct i2c_device_id ov5648_sunrise_i2c_id[] = {
	{OV5648_SUNRISE_SENSOR_NAME,
		(kernel_ulong_t)&ov5648_sunrise_s_ctrl},
	{ }
};

static int32_t msm_ov5648_sunrise_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	return msm_sensor_i2c_probe(client, id, &ov5648_sunrise_s_ctrl);
}

static struct i2c_driver ov5648_sunrise_i2c_driver = {
	.id_table = ov5648_sunrise_i2c_id,
	.probe  = msm_ov5648_sunrise_i2c_probe,
	.driver = {
		.name = OV5648_SUNRISE_SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client ov5648_sunrise_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_WORD_ADDR,
};

// add by yangze for camera sensor otp func test (x825) 2013-08-21 begin
struct st_ov5648_sunrise_otp{
...
struct st_ov5648_sunrise_otp st_ov5648_sunrise_default_otp={
...
#define BG_Ratio_Typical_ov5648_sunrise 0x0129
#define RG_Ratio_Typical_ov5648_sunrise 0x013E
// modified by yangze for ov5648 AWB typical value (x825) 2013-08-28 end

#define OTP_DEBUG_EN 0

static int16_t ov5648_sunrise_check_otp(struct msm_sensor_ctrl_t *s_ctrl, int16_t index){
	int16_t flag, i;
	int16_t rg, bg;
...
static int16_t ov5648_sunrise_read_otp(struct msm_sensor_ctrl_t *s_ctrl, int16_t index, struct st_ov5648_sunrise_otp *otp_ptr){
	int16_t i;
	int16_t temp1, temp2;
...
static int16_t ov5648_sunrise_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl, int16_t R_gain, int16_t G_gain, int16_t B_gain){
...
static void ov5648_sunrise_start_setting(struct msm_sensor_ctrl_t * s_ctrl){
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,0x0100,0x01,MSM_CAMERA_I2C_BYTE_DATA);
}

static void ov5648_sunrise_stop_setting(struct msm_sensor_ctrl_t * s_ctrl){
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client,0x0100,0x00,MSM_CAMERA_I2C_BYTE_DATA);
}	

static int16_t ov5648_sunrise_update_otp_awb(struct msm_sensor_ctrl_t * s_ctrl){
	struct st_ov5648_sunrise_otp current_otp;
	int16_t i;
	int16_t otp_index;
	int16_t temp;
	int16_t R_gain;
	...
	for(i=1;i<=3;i++){
		temp = ov5648_sunrise_check_otp(s_ctrl, i);
		if(temp == 2){
			otp_index = i;
			break;
		}
	...
	if(i>3){
		current_otp.module_integrator_id = st_ov5648_sunrise_default_otp.module_integrator_id;
		current_otp.lens_id = st_ov5648_sunrise_default_otp.lens_id;
		current_otp.rg_ratio = st_ov5648_sunrise_default_otp.rg_ratio;
		current_otp.bg_ratio = st_ov5648_sunrise_default_otp.bg_ratio;
		current_otp.user_data[0] = st_ov5648_sunrise_default_otp.user_data[0];
		current_otp.user_data[1] = st_ov5648_sunrise_default_otp.user_data[1];
		current_otp.light_rg = st_ov5648_sunrise_default_otp.light_rg;
		current_otp.light_bg= st_ov5648_sunrise_default_otp.light_bg;
	}else{
		ov5648_sunrise_read_otp(s_ctrl, otp_index, &current_otp);
	}
	...
	if(bg<BG_Ratio_Typical_ov5648_sunrise){
		if(rg<RG_Ratio_Typical_ov5648_sunrise){
			G_gain = 0x400;
			B_gain = 0x400*BG_Ratio_Typical_ov5648_sunrise/bg;
			R_gain = 0x400*RG_Ratio_Typical_ov5648_sunrise/rg;
		}else{
			R_gain = 0x400;
			G_gain = 0x400*rg/RG_Ratio_Typical_ov5648_sunrise;
			B_gain = G_gain*BG_Ratio_Typical_ov5648_sunrise/bg;
		}	
	}else{
		if(rg<RG_Ratio_Typical_ov5648_sunrise){
			B_gain = 0x400;
			G_gain = 0x400*bg/BG_Ratio_Typical_ov5648_sunrise;
			R_gain = G_gain*RG_Ratio_Typical_ov5648_sunrise/rg;
		}else{
			G_gain_B = 0x400*bg/BG_Ratio_Typical_ov5648_sunrise;
			G_gain_R = 0x400*rg/RG_Ratio_Typical_ov5648_sunrise;
	...
	ov5648_sunrise_update_awb_gain(s_ctrl, R_gain, G_gain, B_gain);
...
static void ov5648_sunrise_otp_init_setting(struct msm_sensor_ctrl_t *s_ctrl){
...
static void ov5648_sunrise_update_otp(struct msm_sensor_ctrl_t *s_ctrl){
	...
	ov5648_sunrise_start_setting(s_ctrl);

	rc = ov5648_sunrise_update_otp_awb(s_ctrl);
	if(rc == 0){
	...
	ov5648_sunrise_stop_setting(s_ctrl);
	// add by yangze for camera sensor otp func (x825) 2013-08-21 end

	return;
}

static struct msm_sensor_fn_t ov5648_sunrise_sensor_func_tbl = {
	.sensor_config = msm_sensor_config,
	.sensor_power_up = msm_sensor_power_up,
	.sensor_power_down = msm_sensor_power_down,
	.sensor_match_id = msm_sensor_match_id,
	.sensor_otp_init_setting = ov5648_sunrise_otp_init_setting,
	.sensor_update_otp = ov5648_sunrise_update_otp,
};
// add by yangze for camera sensor otp func test (x825) 2013-08-19 end

static struct msm_sensor_ctrl_t ov5648_sunrise_s_ctrl = {
	.sensor_i2c_client = &ov5648_sunrise_sensor_i2c_client,
	.power_setting_array.power_setting = ov5648_sunrise_power_setting,
	.power_setting_array.size =
			ARRAY_SIZE(ov5648_sunrise_power_setting),
	.msm_sensor_mutex = &ov5648_sunrise_mut,
	.sensor_v4l2_subdev_info = ov5648_sunrise_subdev_info,
	.sensor_v4l2_subdev_info_size =
			ARRAY_SIZE(ov5648_sunrise_subdev_info),
	// add by yangze for camera sensor otp func test (x825) 2013-08-19
	.func_tbl = &ov5648_sunrise_sensor_func_tbl,			
};

static const struct of_device_id ov5648_sunrise_dt_match[] = {
	{
		.compatible = "qcom,ov5648_sunrise",
		.data = &ov5648_sunrise_s_ctrl
	},
	{}
};

MODULE_DEVICE_TABLE(of, ov5648_sunrise_dt_match);

static struct platform_driver ov5648_sunrise_platform_driver = {
	.driver = {
		.name = "qcom,ov5648_sunrise",
		.owner = THIS_MODULE,
		.of_match_table = ov5648_sunrise_dt_match,
	},
};

static int32_t ov5648_sunrise_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	...
	match = of_match_device(ov5648_sunrise_dt_match, &pdev->dev);
	rc = msm_sensor_platform_probe(pdev, match->data);

	// added by yangze for camera sensor hardware_info 2013-08-30 begin
	#ifdef CONFIG_GET_HARDWARE_INFO
	if(rc == 0){
		register_hardware_info(SUB_CAM, ov5648_sunrise_hardware_info);
	}
	#endif
	// added by yangze for camera sensor hardware_info 2013-08-30 end
...
static int __init ov5648_sunrise_init_module(void)
{
	int32_t rc = 0;

	rc = platform_driver_probe(&ov5648_sunrise_platform_driver,
		ov5648_sunrise_platform_probe);
	if (!rc)
		return rc;
	return i2c_add_driver(&ov5648_sunrise_i2c_driver);
}

static void __exit ov5648_sunrise_exit_module(void)
{
	if (ov5648_sunrise_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&ov5648_sunrise_s_ctrl);
		platform_driver_unregister(&ov5648_sunrise_platform_driver);
	} else
		i2c_del_driver(&ov5648_sunrise_i2c_driver);
	return;
}

module_init(ov5648_sunrise_init_module);
module_exit(ov5648_sunrise_exit_module);
MODULE_DESCRIPTION("ov5648_sunrise");
