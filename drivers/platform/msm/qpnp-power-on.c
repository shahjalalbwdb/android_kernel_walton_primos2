...
//Added by zhaochengliang for disable power key reset (X825) SW000000 2013/10/21 begin
		else{
			if(cfg->s2_cntl2_addr){
				/* disable reset */
				rc = qpnp_pon_masked_write(pon, cfg->s2_cntl2_addr,
								QPNP_PON_S2_CNTL_EN, 0);			
				dev_info(&pon->spmi->dev, "disable %d S2 enable\n", cfg->pon_type);

			}

		}
//Added by zhaochengliang for disable power key reset (X825) SW000000 2013/10/21 end
