...
static unsigned char CTPM_FW_BYD[] = {
	#include "HQ_X825_5336_Byd0x59_Ver0x0A_20130816_app.h"
};
static unsigned char CTPM_FW_CHAOSHENG[] = {
	#include "ver0x0a_20130923_11204_x825b_id0x57_chaosheng_app.h"
};
...
upgrade with *.i file
*/
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client,unsigned char CTPM_FW[],int size)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	int fw_len = size;
...
u8 fts_ctpm_get_i_file_ver(unsigned char CTPM_FW[],u16 size)
{
	u16 ui_sz;
	ui_sz = size;
	if (ui_sz > 2)
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	fts_read_reg(client, FTS_REG_VENDOR_ID, &cur_vendor_id);

       printk("[FTS] cur_vendor_id = 0x%x\n",cur_vendor_id);
	if(0x59==cur_vendor_id){
	    CTPM_FW=CTPM_FW_BYD;
	    fw_size=sizeof(CTPM_FW_BYD);
	}
	else if(0x57==cur_vendor_id){
	    CTPM_FW=CTPM_FW_CHAOSHENG;
	    fw_size=sizeof(CTPM_FW_CHAOSHENG);
	}
	else{
	    if(fts_read_maker_id(client, maker_id) < 0){
		    printk("maker id read failed\n");
	           return -EIO;
	    }
           printk("[FTS] maker_id from boot zone is 0x%x\n",maker_id[0]);
	    if(0x59==maker_id[0]){
	        CTPM_FW=CTPM_FW_BYD;
	        fw_size=sizeof(CTPM_FW_BYD);
	    }
	    else if(0x57==maker_id[0]){
	        CTPM_FW=CTPM_FW_CHAOSHENG;
	        fw_size=sizeof(CTPM_FW_CHAOSHENG);
	    }
	    else{
               printk("[FT5x36],invalid vendor id 0x%x,stop updating...\n",maker_id[0]);
	        return -EIO;
	    }
	}
	uc_host_fm_ver = fts_ctpm_get_i_file_ver(CTPM_FW,fw_size);
	...
	if (/*the firmware in touch panel maybe corrupted */
		...
		i_ret = fts_ctpm_fw_upgrade_with_i_file(client,CTPM_FW,fw_size);
...
int fts_read_maker_id(struct i2c_client * client, char * maker_id) {
	u8 reg_val[2] = {0};
	u32 i = 0;
	u32  temp;
	u8 	packet_buf[4];
...
int fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (DEVICE_IC_TYPE == IC_FT6208 || DEVICE_IC_TYPE == IC_FT6x06)
			...
			fts_write_reg(client, 0xfc, FT_UPGRADE_AA);
		msleep(upgradeinfo.delay_aa + i*10);
		...
		msleep(upgradeinfo.delay_55 + i*2);
		/*********Step 2:Enter upgrade mode *****/
		printk("[FTS] Step 2:Enter upgrade mode \n");
		#if 1
			auc_i2c_write_buf[0] = FT_UPGRADE_55;
...
#ifdef FTS_AUTO_UPDATE
	fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
	uc_host_fm_ver1 = fts_ctpm_get_i_file_ver(CTPM_FW,fw_size);
	 printk("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver1);
