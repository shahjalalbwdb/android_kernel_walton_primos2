					...
					struct usb_composite_dev *cdev)
{
	...
	if (dev->pdata && dev->pdata->cdrom) {
		...
		config->fsg.nluns++;
	}/* removed by Devine enum one sdcard only SW00019426 825 2013/12/02 begin
	if (dev->pdata && dev->pdata->internal_ums) {
		...
		config->fsg.nluns++;
	}*/ /*removed by Devine enum one sdcard only SW00019426 825 2013/12/02 end*/

    config->fsg.luns[0].cdrom = 0;//added  by Devine for enum one sdcard only SW00019426 2013/12/02
    config->fsg.luns[0].ro = 0;//added by Devine for enum one sdcard only SW00019426 2013/12/02
...
static int android_bind(struct usb_composite_dev *cdev)
{
    ...
    /* Modified by zhangrui for USB name (X825C) SWxxxxxx 2013/11/01 */
	strlcpy(manufacturer_string, "XOLO Q1100",
		sizeof(manufacturer_string) - 1);
	strlcpy(product_string, "XOLO Q1100", sizeof(product_string) - 1);
