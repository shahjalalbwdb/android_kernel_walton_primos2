...
//yuquan added for poweroff charging indicator led
...
//aidongdong add for sleep awake chg current begin
extern int lcd_status;
//aidongdong add for sleep awake chg current end
struct qpnp_chg_irq {
	int		irq;
...
//aidongdong modify for overheat-overcold charging begin
static struct wake_lock 	dc_chg_wake_lock;
static bool dc_chg_lock=false;
static int overheat_cold_tag =0x00;
//aidongdong modify for overheat-overcold charging end
/**
...
//aidongdong add for sleep awake chg current begin
	struct delayed_work		batt_temp_work;
...
/*xiongzuan add for not charger icon when lower battery capacity . 2013/11/14 begin */
static int cur_capacity=0;
/*xiongzuan add for not charger icon when lower battery capacity . 2013/11/14 end */
//aidongdong add for sleep awake chg current begin
static bool dc_chg = false;
//aidongdong add for sleep awake chg current end
enum usbin_health {
	USBIN_UNKNOW,
	USBIN_OK,
...
//aidongdong add for warm cool imax 20140104 begin
#define QPNP_CHG_I_WARM_COOL		600
//aidongdong add for warm cool imax 20140104 end
static int
...
//aidongdong modify for usb state update 20140103 begin
	if (qpnp_chg_is_usb_chg_plugged_in(chip) == 0) {
		pr_info("chg_plug_in =0,set power_supply_set_present\n");
		power_supply_set_present(chip->usb_psy,0);
		power_supply_set_online(chip->usb_psy, 0);
	}
//aidongdong modify for usb state update 20140103 end
}
...
//aidongdong modify for overheat-overcold charging begin
#define TEMP_CHECK_PERIOD_MS 20000
//aidongdong modify for overheat-overcold charging end
#define EOC_CHECK_PERIOD_MS	10000
//aidongdong add for sleep awake chg current begin
#define CURRENT_CHECK_PERIOD_MS	20000
//aidongdong add for sleep awake chg current end
static irqreturn_t
...
qpnp_chg_usb_chg_gone_irq_handler(int irq, void *_chip)
{
	...
	//yuquan added begin for poweroff charging indicator led
	if (!qpnp_chg_is_usb_chg_plugged_in(chip)) {
       ...
       //yuquan added end for poweroff charging indicator led
 //aidongdong add for sleep awake chg current begin
	dc_chg = false ;
...
//aidongdong modify for usb state update 20131229 end
	return IRQ_HANDLED;
}
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (!usb_present) {
			...
			schedule_delayed_work(&chip->eoc_work,
				msecs_to_jiffies(EOC_CHECK_PERIOD_MS));
//aidongdong modify for overheat-overcold charging begin
			schedule_delayed_work(&chip->batt_temp_work,
				msecs_to_jiffies(TEMP_CHECK_PERIOD_MS));
//aidongdong modify for overheat-overcold charging end	
	...
	schedule_delayed_work(&chip->current_work,
				msecs_to_jiffies(CURRENT_CHECK_PERIOD_MS));
	//aidongdong add for sleep awake chg current end
...
//aidongdong modify for DC charge current begin
//#define USB_WALL_THRESHOLD_MA	500
#define USB_WALL_THRESHOLD_MA	1100
//aidongdong modify for DC charge current end
...
get_prop_batt_status(struct qpnp_chg_chip *chip)
{
	...
	if ((qpnp_chg_is_usb_chg_plugged_in(chip) ||qpnp_chg_is_dc_chg_plugged_in(chip)) 
		&& (cur_capacity< 8)&&(get_prop_batt_present(chip))) {
			return POWER_SUPPLY_STATUS_CHARGING;
	}
	/*xiongzuan add for not charger icon when lower battery capacity . 2013/11/14 end */
...
//aidongdong add for SW00025534 when pc sleep led no brightness begin
...
//aidongdong delete for SW00019619 when pc sleep cannot charge being
#if 0						
			qpnp_chg_usb_suspend_enable(chip, 1);
			qpnp_chg_iusbmax_set(chip, QPNP_CHG_I_MAX_MIN_100);
#endif
//aidongdong delete for SW00019619 when pc sleep cannot charge end
		} else {
...
//aidongdong modify for overheat-overcold charging begin
#define BATT_TEMP_OVERHEAT   (57*10)
#define BATT_TEMP_OVERCOLD   (-10*10)
#define BATT_TEMP_COOL_DEGC        (0*10)
#define BTTE_TEMP_WARM_DEGC     (45*10)
static void
qpnp_batt_temp_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, batt_temp_work);
	...
	/*xiongzuan disable cool or warm  chg_current by 20131226 begin*/
	if ((batt_temp < BATT_TEMP_COOL_DEGC)||(batt_temp >= BATT_TEMP_OVERHEAT))
		pr_err("batt_temp = %d \n", batt_temp);
	/*xiongzuan disable cool or warm  chg_current by 20131226 end*/
	if((batt_temp >= BATT_TEMP_OVERHEAT) || (batt_temp <= BATT_TEMP_OVERCOLD)) {
			/* disable charging */
				qpnp_chg_charge_en(chip, 0);
				overheat_cold_tag = 0x55;
				pr_debug("psy changed batt_psy\n");
				power_supply_changed(&chip->batt_psy);
		...
		/* enable charging */
		overheat_cold_tag = 0x00;
#if 0
//aidongdong modify for cooltemp charging current 20140106 begin		
		if ((batt_temp < BATT_TEMP_COOL_DEGC) || (batt_temp > BTTE_TEMP_WARM_DEGC)){
			chip->bat_is_cool = true;
			chip->cool_bat_chg_ma = QPNP_CHG_I_WARM_COOL;
			qpnp_chg_set_appropriate_battery_current(chip);
			pr_debug("qpnp_chg_iusbmax_set 150mA\n");
			}
		else {
			if ((batt_temp >= BATT_TEMP_COOL_DEGC)  && (batt_temp < BTTE_TEMP_WARM_DEGC)) {
...
//aidongdong modify for cooltemp charging current  20140106 end
#endif
		if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
			power_supply_set_online(chip->usb_psy, 1);
			}
...
//aidongdong modify for overheat-overcold charging end
//aidongdong add for sleep awake chg current begin
#define USB_WALL_WAKE_CURRENT   400
//when lcd sleep lcd_status =0;lcd awake lcd_status =1
static void
qpnp_current_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, current_work);
	static bool ibat_max_1100 = false;
	static bool ibat_max_400 = false;
 	if (!dc_chg) {
		ibat_max_1100 = false;
		ibat_max_400  = false;
		goto stop_current;
	...
	if ((lcd_status == 1) && (!ibat_max_400)){
		if (!ext_ovp_present)
			qpnp_chg_iusbmax_set(chip,USB_WALL_WAKE_CURRENT);
		else
	...
	else if ((lcd_status == 0) && (!ibat_max_1100)) {
		if (!ext_ovp_present)
...
qpnp_eoc_work(struct work_struct *work)
{
	...
	capacity_done= get_prop_capacity(chip);
	pm_stay_awake(chip->dev);
	qpnp_chg_charge_en(chip, !chip->charging_disabled);
...
//aidongdong modify for overheat-overcold charging begin
	INIT_DELAYED_WORK(&chip->batt_temp_work, qpnp_batt_temp_work);
//aidongdong modify for overheat-overcold charging end
//aidongdong add for sleep awake chg current begin
	INIT_DELAYED_WORK(&chip->current_work, qpnp_current_work);
//aidongdong add for sleep awake chg current end
...
//aidongdong modify for overheat-overcold charging begin
	cancel_delayed_work_sync(&chip->batt_temp_work);
//aidongdong modify for overheat-overcold charging end	
