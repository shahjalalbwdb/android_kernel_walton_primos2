...
/*xiongzuan add for battery capacity jump by 20131220 bagin*/
#define MIN_OCV_UV		2000000
static int read_soc_params_raw(struct qpnp_bms_chip *chip,
				struct raw_soc_params *raw,
				int batt_temp)
{
	int warm_reset, rc,temp_last_ocv_uv;
	...
	if (chip->prev_last_good_ocv_raw == OCV_RAW_UNINITIALIZED) {
		...
		temp_last_ocv_uv = estimate_ocv(chip);
		pr_debug("temp_last_ocv_uv = %d, temp_last_ocv_uv - chip->last_ocv_uv= %d\n",
				temp_last_ocv_uv, temp_last_ocv_uv - chip->last_ocv_uv);
		if((temp_last_ocv_uv - chip->last_ocv_uv) > 200000)
		{
			raw->last_good_ocv_uv = 1000000;
		}
		pr_debug("PON_OCV_UV = %d, cc = %llx\n",
				chip->last_ocv_uv, raw->cc);
		...
		pr_info("chip->prev_last_good_ocv_raw !=raw->last_good_ocv_raw\n");
	//aidongdong modify for Soc 100 to 1 at 20131223 begin
		rc = qpnp_read_wrapper(chip, (u8 *)&raw->last_good_ocv_raw,
			chip->base + BMS1_OCV_FOR_SOC_DATA0, 2);
	if (raw->last_good_ocv_raw > VADC_INTRINSIC_OFFSET) {
		convert_and_store_ocv(chip, raw, batt_temp);
	...
	//aidongdong modify for Soc 100 to 1 at 20131223 end 
	} else {
		raw->last_good_ocv_uv = chip->last_ocv_uv;
	}
/*xiongzuan add for battery capacity jump by 20131220  end*/
						...
						int batt_temp)
{
	...
	params->rbatt_mohm = get_rbatt(chip, soc_rbatt, batt_temp);
	 //xiongzuan add return 150 when Get error values of rbatt_mohm by 20140122 begin 
		 if (params->rbatt_mohm < 50 || params->rbatt_mohm > 1000)
		 {    
		 	pr_info(" Get error values of rbatt_mohm = %d\n", params->rbatt_mohm);
			params->rbatt_mohm = 150;
		 }
		  //xiongzuan add return 150 when Get error values of rbatt_mohm by 20140122 end 
...
//aidongdong modify for charging soc 	decrease at 20131226 begin
	if (charging) {
		if (chip->last_soc > soc)
			soc = chip->last_soc;
		}
//aidongdong modify for charging soc 	decrease at 20131226 end
...
//aidongdong add for charging soc state at 20131226 begin
	chip->prev_chg_soc = soc;
//aidongdong add for charging soc state at 20131226 end 
							...
							int soc, int batt_temp)
{
	...
	if ((is_battery_charging(chip)) && (ibat_ua < 0)) {
//aidongdong modify for cv time too long at 20131226 begin		
		pr_debug("ibat_ua < 0,charging\n");
		ocv_est_uv = vbat_uv + (ibat_ua * chip->default_rbatt_mohm)/1000;
		pc_est = calculate_pc(chip, ocv_est_uv, batt_temp);
		soc_est = div_s64((s64)params->fcc_uah * pc_est - params->uuc_uah*100,
				(s64)params->fcc_uah - params->uuc_uah);
		soc_est = bound_soc(soc_est);
		if(soc >soc_est){
			pr_debug("soc >soc_est,charging\n");
			soc = soc_est;
			}
		soc = charging_adjustments(chip, params, soc, vbat_uv, ibat_ua,
				batt_temp);
		pr_debug("ibat_ua=%d,vbat_uv=%d,ocv_est_uv=%d,soc_est=%d,rbatt_mohm=%d\n",ibat_ua,vbat_uv,ocv_est_uv,soc_est,params->rbatt_mohm);
		
//aidongdong modify for cv time too long at 20131226 end	
...
static bool is_battery_replaced_in_offmode(struct qpnp_bms_chip *chip)
{
	u8 batt_pres;
	int rc;
	 /*xiongzuan add for remove battery checkout by 20140126 bagin*/
	if (chip->batt_pres_addr == 0)
	{     
	      chip->batt_pres_addr = 0x1208 ;
	}
	 /*xiongzuan add for remove battery checkout by 20140126 end*/
